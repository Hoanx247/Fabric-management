...
#Region " Hien thi len tung trang cua Datagrid"
    ...
    Private Sub Filterdata_Stored()
        ...
        sbXMLString.Append(" />")
        sbXMLString.Append("</Root>")
        _dt = New ListMenuDTO("[VpsXmlList_MaMay_Nhom_UpSet]", sbXMLString.ToString, "select")
...
#Region "EXECUTE-DELETE"
    ...
    Private Sub Xoa(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If dgvr Is Nothing Or dgvr.IsDetailRow = True Then
            Exit Sub
        End If
        Dim EmpID As String = dgvr.Cells("nhommay_id").Value.ToString
        Dim Code As String = dgvr.Cells("nhommay").Value.ToString
        Dim CodeName As String = dgvr.Cells("nhommay").Value.ToString
        Dim qs = MsgBox("Bạn chắc chắn muốn xóa tất cả thông tin về:" _
                        & vbCrLf & vbCrLf & " - Nhóm máy: " & Code,
        ...
        If qs = vbYes Then
            ...
            sbXMLString.Append("<Root>")
            sbXMLString.Append("<DATA ")
            sbXMLString.Append("nhommay_id='" + ReplaceTextXML(EmpID) + "' ")
            sbXMLString.Append(" />")
            sbXMLString.Append("</Root>")
            '//
            _dt = New ListMenuDTO("[VpsXmlList_MaMay_nhom_UpSet]", sbXMLString.ToString, "delete")
...
#End Region
    ...
    Private Sub Super_Dgv_CellClick(ByVal sender As Object, ByVal e As GridCellClickEventArgs)
        ...
        Try
            If dgvr Is Nothing Then
                ...
                Exit Sub
            Else
                _Lst_ID = dgvr.Cells(_colnhommay_id).Value.ToString
                txtnhom.Text = dgvr.Cells(_colnhommay).Value.ToString
    ...
    Private Sub Update_Data(ByVal sqlcommand As String, ByVal command As String)
        If CheckData() = True Then
            ...
            sbXMLString.Append("<Root>")
            sbXMLString.Append("<DATA ")
            sbXMLString.Append("nhommay_id='" + ReplaceTextXML(_Lst_ID) + "' ")
            sbXMLString.Append("nhommay='" + ReplaceTextXML(txtnhom.Text) + "' ")
            sbXMLString.Append("ghichu='" + ReplaceTextXML(txtghichu.Text) + "' ")
            sbXMLString.Append("nguoitao='" + ReplaceTextXML(_stUser_Save) + "' ")
            sbXMLString.Append(" />")
            sbXMLString.Append("</Root>")
            '//
            _dt = New ListMenuDTO("[VpsXmlList_MaMay_nhom_UpSet]", sbXMLString.ToString, command)
            _update_ok = dtControler.UPSET_XML(_dt)
            If _update_ok = True Then
                Ischanged = True
                '//
...
#Region "TAO MA ID"
    Private Sub Tao_MaID()
        ...
        For y = 1 To 1000
            _stt = "00000" & y
            _LID = "G" & gRight(_stt, 4)
            If SearchUsingForLoop(_LID, "nhommay_id") = False Then
                GoTo SWERLExit
                Exit Sub
            End If
        Next
