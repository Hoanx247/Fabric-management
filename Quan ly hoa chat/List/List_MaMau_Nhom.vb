...
#Region " Hien thi len tung trang cua Datagrid"
    ...
    Private Sub Filterdata_Stored()
        ...
        sbXMLString.Append(" />")
        sbXMLString.Append("</Root>")
        _dt = New ListMenuDTO("[VpsXmlList_MaMau_nhom_UpSet]", sbXMLString.ToString, "select")
...
#Region "EXECUTE-DELETE"
    ...
    Private Sub Xoa(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If dgvr Is Nothing Or dgvr.IsDetailRow = True Then
            Exit Sub
        End If
        Dim EmpID As String = dgvr.Cells("nhommau_id").Value.ToString
        Dim Code As String = dgvr.Cells("nhommau").Value.ToString
        Dim CodeName As String = dgvr.Cells("nhommau").Value.ToString
        ...
        If qs = vbYes Then
            ...
            sbXMLString.Append("<Root>")
            sbXMLString.Append("<DATA ")
            sbXMLString.Append("nhommau_id='" + ReplaceTextXML(EmpID) + "' ")
            sbXMLString.Append(" />")
            sbXMLString.Append("</Root>")
            '//
            _dt = New ListMenuDTO("[VpsXmlList_MaMau_nhom_UpSet]", sbXMLString.ToString, "delete")
...
#End Region
    ...
    Private Sub Super_Dgv_CellClick(ByVal sender As Object, ByVal e As GridCellClickEventArgs)
        ...
        Try
            If dgvr Is Nothing Then
                ...
                Exit Sub
            Else
                _Lst_ID = dgvr.Cells("nhommau_id").Value.ToString
                txtnhom.Text = dgvr.Cells("nhommau").Value.ToString
    ...
    Private Sub Update_Data(ByVal sqlcommand As String, ByVal command As String)
        If CheckData() = True Then
            ...
            sbXMLString.Append("<Root>")
            sbXMLString.Append("<DATA ")
            sbXMLString.Append("nhommau_id='" + ReplaceTextXML(_Lst_ID) + "' ")
            sbXMLString.Append("nhommau='" + ReplaceTextXML(txtnhom.Text) + "' ")
            sbXMLString.Append("ghichu='" + ReplaceTextXML(txtghichu.Text) + "' ")
            sbXMLString.Append("nguoitao='" + ReplaceTextXML(_stUser_Save) + "' ")
            sbXMLString.Append(" />")
            sbXMLString.Append("</Root>")
            '//
            _dt = New ListMenuDTO("[VpsXmlList_MaMau_nhom_UpSet]", sbXMLString.ToString, command)
            _update_ok = dtControler.UPSET_XML(_dt)
            If _update_ok = True Then
                Ischanged = True
                '//
