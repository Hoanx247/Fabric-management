...
Public Class frmTimeLine_NhanCong
    ...
    Private Sub Me_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim workStartTime As WorkTime = New WorkTime(7, 0)
        Dim workEndTime As WorkTime = New WorkTime(19, 0)
...
#End Region
    ...
    Private Sub ADD_LichCang()
        ...
        Dim _IsOvertime_loop_01 As Boolean = False
        Dim _IsOvertime_loop_02 As Boolean = False
        Dim _Loop_hoanthanh_02 As Integer = 0
        Dim _Loop_hoanthanh_03 As Integer = 0
        ...
        For _stt = 0 To defUsers.Length - 1
            ...
            _Loop_hoanthanh_02 = 0
            _IsOvertime_loop_01 = False
            _IsOvertime_loop_02 = False
            _intMinute = 0
            '//
            _Endtime_KetThuc = _TimeServer
            'Dgv_LichCang_May.DataSource = New DataView(dt_local, "mamay='" & defUsers(_stt) & "'", "hoanthanh asc,thutu_sanxuat asc", DataViewRowState.CurrentRows)
            '////
            For Each row As DataGridViewRow In Dgv_LichCang.Rows
                If row.Cells("mamay").Value.ToString = defUsers(_stt) Then
                    ...
                    _sSubject = String.Format(stInfo, row.Cells(_colMame).Value.ToString.ToUpper,
                                                      row.Cells(_coltencongdoan).Value.ToString.ToUpper)
                    ...
                    _sToolTip = "Đơn Hàng: " & row.Cells(_coldonhang).Value.ToString() & vbCrLf
                    _sToolTip &= "Mẻ Ghép: " & row.Cells("mame_ghep").Value.ToString() & vbCrLf
                    ...
                    '_sToolTip &= "Ghi Chú ĐĐSX: " & row.Cells("ghichu_dieudosanxuat").Value.ToString() & vbCrLf
                    ...
                    _stag &= row.Cells("mame").Value.ToString.ToUpper & ";"
                    _stag &= row.Cells("congdoan_id").Value.ToString.ToUpper & ";"
                    _stag &= row.Cells("tencongdoan").Value.ToString.ToUpper & ";"
                    ...
                    ElseIf row.Cells("hoanthanh").Value = 1 Then
                        ...
                        _Endtime_dutinh = _StartTime.AddMinutes(_Thoigian_cho)

                        '//So Sanh voi thoi gian hien tai
                        If _Endtime_dutinh <= _TimeServer Then
                            _IsOvertime_loop_01 = True
                            _Endtime_KetThuc = _TimeServer
                        Else
                            _IsOvertime_loop_01 = False
                        ...
                        End If
                        '//
                        _Thoigian_cho = CInt(row.Cells("thoigian_cho").Value)
                        '//
                        _sToolTip &= "Giờ Vào: " & row.Cells("thoigianvao").Value.ToString() & vbCrLf
                        _sToolTip &= "Giờ Ra: "
                    ...
                    ElseIf row.Cells("hoanthanh").Value = 2 Then '//MẺ CÓ KẾ HOẠCH
                        'Dim _thoigian_cho As Integer = 0

                        If _Loop_hoanthanh_02 = 0 Then
                            _Loop_hoanthanh_02 = 1
                            If _Thoigian_cho = 0 Then _Thoigian_cho = 20
                            ...
                            Else
                                If _IsOvertime_loop_01 = False Then
                                    _StartTime = _Endtime_KetThuc.AddMinutes(_Thoigian_cho)
                                    _Endtime_KetThuc = _StartTime.AddMinutes(_thoigian_toithieu)
                                Else
                                    _StartTime = _Endtime_KetThuc.AddMinutes(_Thoigian_cho)
                        ...
                        Else
                            _StartTime = _Endtime_KetThuc.AddMinutes(_Thoigian_cho)
                            _Endtime_KetThuc = _Endtime_KetThuc.AddMinutes(_thoigian_toithieu)
                            _Thoigian_cho = CInt(row.Cells("thoigian_cho").Value)
                    ...
                    ElseIf row.Cells("hoanthanh").Value = 3 Then '// MẺ CHƯA CÓ KẾ HOẠCH
                        '//
                        If _Thoigian_cho = 0 Then _Thoigian_cho = 20
                        _StartTime = _Endtime_KetThuc.AddMinutes(_Thoigian_cho)
                        _Endtime_KetThuc = _StartTime.AddMinutes(_thoigian_toithieu)
                        ...
                        Dim ap As Appointment = AddAppointment_DuKien(_sSubject, _smamau, _StartTime,
                                           _Endtime_KetThuc,
                            defUsers(_stt), _color_me, Appointment.TimerMarkerBusy, _sToolTip, _stag)
                    End If
                End If
            Next
...
#Region "calendarView1_ItemDoubleClick"
    ...
    Private Sub calendarView1_ItemDoubleClick(ByVal sender As Object, ByVal e As MouseEventArgs) Handles CalendarView1.ItemDoubleClick
        ...
        If item IsNot Nothing Then
            ...
            LID = _List_Data(0)
            _Mame = _List_Data(1)
            _LCongDoan_id = _List_Data(2)
            _LMaCongDoan = _List_Data(3)
            _LMamay_id = _List_Data(4)
