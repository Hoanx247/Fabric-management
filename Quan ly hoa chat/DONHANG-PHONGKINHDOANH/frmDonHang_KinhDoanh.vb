...
Public Class frmDonHang_KinhDoanh
...
#Region " Load du liệu lên bảng khi mở Form"
    ...
    Private Sub Me_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '//
        ChkTatCa.CheckState = CheckState.Checked
    End Sub
...
#Region "LOAD DATA"
    ...
    Private Sub Filterdata_chung()
        ...
        sbXMLString.Append(" />")
        sbXMLString.Append("</Root>")
        If ChkTatCa.CheckState = CheckState.Unchecked Then
            _dt = New KhoSanXuatDTO("[VpsXmlDonHang_Main_GetData_2021]", sbXMLString.ToString, "select")
        Else
...
#End Region
    ...
    Private Sub txt_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtdonhang_F.KeyDown, txtkhachhang_F.KeyDown
...
#End Region
    ...
    Private Sub ChkTatCa_CheckedChanged(sender As Object, e As EventArgs) Handles ChkTatCa.CheckedChanged
        If ChkTatCa.CheckState = CheckState.Checked Then
            dt1_F.Enabled = True
            dt2_F.Enabled = True
        Else
            dt1_F.Enabled = False
    ...
    Private Sub ButtonItem_Enable_Click(sender As Object, e As EventArgs) Handles ButtonItem_Enable.Click
        Dim panel As GridPanel = Super_Dgv.PrimaryGrid
        If MsgBox("Bạn có muốn chọn Enable?", CType(MsgBoxStyle.Question + MsgBoxStyle.YesNo, MsgBoxStyle), "Warning") = MsgBoxResult.No Then
            Exit Sub
        End If
        '//
        LEnable = 1
        ...
        sbXMLString.Append("</Root>")
        '//
        _dt = New KhoSanXuatDTO("VpsXmlDonHang_Main_UpSet_2021", sbXMLString.ToString, "update_enable")
        ...
        For Each item As GridElement In rows
            ...
            Else
                ...
                If row IsNot Nothing AndAlso row.Visible = True AndAlso row.Checked = True AndAlso row.IsDetailRow = False Then
                    ...
                    sbXMLString.Append("<DATA ")
                    sbXMLString.Append("donhang_id='" + ReplaceTextXML(row.Cells("donhang_id").Value.ToString) + "' ")
                    sbXMLString.Append("isactive='" + CNumber_system(LEnable) + "' ")
                    sbXMLString.Append(" />")
                End If
            End If
        Next
    ...
    Private Sub ButtonItem_Disable_Click(sender As Object, e As EventArgs) Handles ButtonItem_Disable.Click
        Dim panel As GridPanel = Super_Dgv.PrimaryGrid
        If MsgBox("Bạn có muốn chọn Disable?", CType(MsgBoxStyle.Question + MsgBoxStyle.YesNo, MsgBoxStyle), "Warning") = MsgBoxResult.No Then
            Exit Sub
        End If
        '//
        LEnable = 0
...
#Region " VIEW ORDER"
    Private Sub ShowModalForm_DonHang_View()
        If Me.InvokeRequired Then
            Me.BeginInvoke(New Action(AddressOf ShowModalForm_DonHang_View))
        Else
            'Using f As frmKhoTong_Nhapkhovai_1input ' = New frmKhoTong_Nhapkhovai_1input With {.MaximizeBox = False, .MinimizeBox = False}
            With frmDonHang_View_DonHang
                .DonHang = Ldonhang
    ...
    Private Sub ButtonItem_View_DonHang_Click(sender As Object, e As EventArgs) Handles ButtonItem_View_DonHang.Click
        ShowModalForm_DonHang_View()
    End Sub
